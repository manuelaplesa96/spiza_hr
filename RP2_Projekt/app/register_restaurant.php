...
try
{
	$st = $db->prepare( 'SELECT * FROM spiza_restaurants WHERE registration_sequence=:reg_seq' );
...
else
{
	...
	try
	{
		$st = $db->prepare( 'UPDATE spiza_restaurants SET has_registered=1 WHERE registration_sequence=:reg_seq' );
		$st->execute( array( 'reg_seq' => $_GET['niz'] ) );
	}
	catch( PDOException $e ) { exit( 'Greška u bazi: ' . $e->getMessage() ); }
